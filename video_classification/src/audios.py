...
while(1):
    ...
    sounddevice.wait()
    write(Path('./audio/', '{}.wav'.format(str(count))), fs, record_voice)

    # Load in audio file
    data, sr = librosa.load(Path('./audio/', '{}.wav'.format(str(count))))
    ...
    numpy.save('scaler_mean.npy', scaler.mean_)
    numpy.save('scaler_scale.npy', scaler.scale_)
